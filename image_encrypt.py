...
def deCryption(image_to_decrypt, key, pixel_array_nocypher):
    # La funzione prende in input:
    # image_to_decrypt: una stringa che rappresenta l'immagine da procesare (esempio: "image_encrypted.png")
    # key: un numero che rappresenta la chiave di cifratura/shuffling e che deve necessariamente coincidere con
    # il valore inserito nella cifratura (esempio: 123456789)
    # pixel_array_nocypher: tutti i pixel dell'immagine non cifrati
    ...
    for i in range(0, rows, side):
        for j in range(0, cols, side):
            ...
            block_index += 1
    image = Image.fromarray(output_array_cypher)
    image.save("image_decypher.png")
    image.close()

    return "image_decypher.png"
